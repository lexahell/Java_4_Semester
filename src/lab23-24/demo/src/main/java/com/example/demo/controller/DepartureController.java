...
public class DepartureController {
    ...
    public Departure showDepatureByType(@PathVariable String type){
        return departureService.getDepartureByType(type);
    }

    @GetMapping("/showbydate/{date}")
    public List<Departure> showDeparturesByDate(@PathVariable @DateTimeFormat(pattern = "yyyy-MM-dd") Date date){
        return departureService.getDeparturesByDepartureDate(date);
    }
}
